...
class QM {
  public:
    int VARIABLES;
		...
		// function to get the boolean terms letters
		...
		*/
		...
		// Eg. If there are 4 variables, 2, that is 10, in binary in represented as 0010
		...
		// function to check if two terms differ by only one bit
		...
		// function to replace complement terms with dont cares
		...
		vector<string> reduce(vector<string> minterms) {
			...
			for(int i = 0; i < max; i++) {
				for(int j = i; j < max; j++) {
					// If a grey code pair is found, replace the differing bits by dontcares.
		...
		// Converting the boolean minterm into variable expression
		// Eg. 011- becomes a'bc
...
int main() {
	...
	while(check == 'y') {
		int no;
		cout << endl << "Enter number of variables" << endl;
		cin >> no;
		if(no > 8 || no < 1) {
